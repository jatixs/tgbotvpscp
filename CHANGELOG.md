...
### 🚀 Добавлено / Улучшено:
* **Система Биллинга (Billing):** Внедрена комплексная система напоминаний об оплате серверов. Бот теперь может автоматически уведомлять администратора об истекающих сроках аренды нод (за 3 дня до окончания) и мастер-сервера.
* **Управление Биллингом (Bot):** В меню управления серверами (как мастер-сервера, так и нод) добавлены кнопки настройки биллинга. Администратор может задать сумму оплаты, выбрать валюту (Евро, Доллар, Рубль) и сдвинуть дату следующего платежа прямо из Telegram.
* **WebUI (Дашборд и Ноды):** В веб-панель интегрировано отображение статуса аренды серверов. В шапке панели и на карточках нод теперь выводятся информативные локализованные бейджи с оставшимся количеством дней (или статусом "Просрочен!").
* **WebUI (Детали Аренды):** Добавлено современное модальное окно с подробностями об аренде сервера (стоимость, дата ближайшего платежа).

### 🔧 Исправлено:
* **WebUI (Безопасность):** Решена проблема конфликта обработчиков кликов и системы очистки `DOMPurify`. События теперь корректно привязываются через JS-скрипты, что возвращает работоспособность модальным окнам.
* **Core (Временные зоны):** Исправлена ошибка в `middlewares.py` (`AttributeError: 'int' object has no attribute 'tzinfo'`), приводившая к сбоям при обработке устаревших callback-запросов.
* **База данных:** В схему БД (`nodes_db.py`) добавлены автоматические миграции для поддержки полей биллинга (`billing_amount`, `currency`, `next_payment_date`, `reminder_enabled`) без потери существующих данных.
