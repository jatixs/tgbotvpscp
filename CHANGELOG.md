...
* **WebUI (Мини-графики):** В блоке быстрой статистики добавлены мини-графики нагрузки CPU/RAM/Диска (по аналогии с графиком трафика), а также наглядная визуализация Uptime/Downtime в виде разделенной шкалы «онлайн/простой».
...
* **WebUI (Адаптивная сетка дашборда):** Блоки в одной строке теперь всегда выравниваются по высоте, а их внутреннее содержимое (список служб, карточки нод, окно системных логов) корректно адаптируется под выделенное пространство и под ручное изменение размера блока, вместо образования пустых «растянутых» областей.
* **WebUI (Карточки нод):** Переработано отображение статистики (ЦП/ОЗУ/Диск/трафик) в карточках нод — компактная сетка вместо инлайн-стилей, корректно выглядит при любом размере блока «Управление Нодами».

### 🔧 Исправлено:
* **WebUI (Добавление ноды):** Устранён конфликт двух реализаций шифрования запросов (`encryptData`) в `common.js` и `dashboard.js` — старая XOR-функция в `dashboard.js` перекрывала корректную AES-функцию, из-за чего сервер не мог расшифровать запрос и возвращал ошибку «Name required» при попытке добавить ноду.
