...
* **Самодиагностика:** В модуль `/selftest` добавлен сбор фактического потребления ресурсов самим процессом бота, а частота CPU теперь форматируется динамически с учетом реальной нагрузки. Добавлена кнопка «Обновить состояние».
...
* **Интерактивность:** Скрипт оптимизации переведен на инлайн-чекбоксы с сохранением состояния. Выполнение Speedtest теперь сопровождается живым счетчиком времени в фоновом режиме с учетом лимитов Telegram API.
