...
### 🚀 Добавлено / Улучшено:
* **Модуль Client Alerts (Gateway Bot):** Полностью переработан механизм связи с пользователями. Теперь клиенты общаются через отдельного бота-шлюза (`ALERT_BOT_TOKEN`). Сообщения пересылаются администратору в главный бот в виде тикетов, на которые можно отвечать напрямую через inline-кнопку «Ответить».
* **Рассылки (Broadcast):** Добавлена возможность отправлять сообщения (текст, фото, видео) всем подписчикам Gateway-бота прямо из панели управления в Telegram.
* **Защита от спама (Anti-Flood):** В Alert-модуль встроен механизм антифлуда. Клиенты могут написать только с помощью функции `Reply` (Ответить) на сообщение от администратора, с ограничением не чаще 1 раза в 3 секунды.
* **Интерфейс администратора:** Убраны устаревшие команды `/broadcast` и `/alert_subscribers`. Все функции вынесены в удобное inline-меню «Панель управления Alert Module».
