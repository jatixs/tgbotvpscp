...
* **Memory Orchestrator:** Внедрена система «Оркестратора» (`core/orchestrator.py`) для динамической выгрузки неиспользуемых модулей из памяти (Garbage Collection). Теперь бот загружает тяжелые модули только по требованию и выгружает их через 5 минут неактивности, экономя оперативную память на слабых VPS.
* **Абсолютные метрики ресурсов:** В модуле `selftest` добавлен сбор фактического потребления CPU/RAM самим процессом бота, чтобы администратор мог точно отслеживать занимаемые ботом ресурсы (`/memstats`).

### ✨ Улучшения UI/UX:
* **Очистка чата (AutoDelete):** Добавлен `AutoDeleteMessageMiddleware`, который мгновенно удаляет текстовые команды и нажатия меню от пользователя, чтобы чат оставался чистым и не захламлялся.
* **Удаление дубликатов (Smart Cleanup):** Оживлена функция `delete_previous_message`. Теперь при повторном вызове команды бот корректно находит и удаляет свой предыдущий ответ на *эту же самую* команду, оставляя навигацию приятной и лаконичной.
* **Защита клавиатуры:** Полностью переписана логика вызова меню, чтобы предотвратить баг клиента Telegram, при котором обычная клавиатура перекрывала кнопки бота. Бот теперь отправляет новое меню *до* удаления старого.
* **Интерактивная оптимизация:** Скрипт "Оптимизации" полностью переписан. Теперь перед выполнением бот предлагает удобное инлайн-меню с чекбоксами (сохраняющими состояние между сессиями), позволяя выбрать конкретные модули для применения (например, пропустить удаление Snapd или тюнинг Nginx).
* **Таймаут инлайн-кнопок:** Добавлен `CallbackTTLMiddleware` (30 секунд). Если попытаться нажать устаревшую инлайн-кнопку, бот мягко уведомит об обновлении данных и удалит старое меню.
* **Менее назойливое приветствие:** Возврат в главное меню теперь сопровождается короткой надписью "📂 Главное меню:" вместо длинного приветствия. Длинное приветствие осталось только для команды `/start`.
* **Пасхалки и Факты:** Если боту отправить неизвестную команду или текст, он перехватит сообщение и ответит случайным интересным фактом. Встроен переводчик (Google Translate API) для перевода фактов на язык пользователя «на лету». Анимация набора текста факта использует метод `sendMessageDraft`.
* **Проверка сервера (Selftest):** Добавлена инлайн-кнопка «🔄 Обновить состояние» для динамического обновления статистики без переотправки сообщения.
* **Форматирование частоты CPU:** В результатах `/selftest` частота процессора теперь форматируется динамически (Hz, KHz, MHz, GHz) и показывает фактически использованную (потребляемую) частоту пропорционально нагрузке.
* **Интерактивный Speedtest:** Внедрены фоновые таймеры для команды Speedtest. Теперь при запуске замера (как локально, так и на удаленной ноде) бот отправляет сообщение с тикающим счетчиком времени, а по завершении заменяет его итоговым отчетом. Учтены лимиты `FloodWait` (Telegram API).
* **WebUI Предупреждения (Алерты):** На дашборде веб-интерфейса добавлены динамические индикаторы предупреждений. Если потребление ресурсов (CPU, RAM, Disk) достигает лимитов из настроек бота, рядом с названием метрики появляется аккуратный предупреждающий знак ⚠️, а полоса прогресса меняет цвет на оранжевый. При пиковых значениях индикаторы становятся красными.
* **Мелкие улучшения UI:** Цвета виджета «Общий трафик» (RX/TX) на дашборде синхронизированы с остальными графиками интерфейса (зеленый для входящего, синий для исходящего).

### 📦 Зависимости и пакеты:
* **Обновление библиотек:** Все Python-пакеты в `requirements.txt` и `Dockerfile` обновлены до последних доступных и взаимно-совместимых версий (например, `aiogram` до 3.28.2, `tortoise-orm` до 0.25.4, `aiohttp` до 3.13.5).

### 📖 Документация:
* **Архитектура и Модули:** Обновлены `ARCHITECTURE.md` и `custom_module.md` для отражения последних архитектурных изменений: добавлен `core/orchestrator.py` (Memory Orchestrator), описаны новые `AutoDeleteMessageMiddleware` и `CallbackTTLMiddleware`, добавлен fallback-обработчик с API uselessfacts, а также актуализирован процесс регистрации новых модулей через `MODULE_CONFIG`.
