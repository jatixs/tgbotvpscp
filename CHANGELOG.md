...
#### 🚀 Добавлено:
* **Многоязычность (i18n):**
    * Добавлена полная поддержка русского и английского языков для всех сообщений бота, включая кнопки, меню, ошибки и уведомления.
    * Пользователи теперь могут выбирать предпочитаемый язык с помощью новой кнопки "🇷🇺 Язык" / "🇬🇧 Language" в главном меню.
    * Настройки языка сохраняются для каждого пользователя в `config/user_settings.json`.
    * Создан новый модуль `core/i18n.py` для управления переводами и языковыми настройками.
* **Обновленная документация:** Добавлены английские версии `README.md` и `CHANGELOG.md` со ссылками для переключения между языками.

#### ✨ Улучшено:
* **Структура кода:** Все текстовые строки вынесены из кода модулей и ядра в центральный словарь переводов `core/i18n.py`, что упрощает добавление новых языков и редактирование текстов.
* **Обработка ошибок Watchdog:** Улучшена обработка ошибок и вывод сообщений в `watchdog.py` с использованием системы i18n (на языке по умолчанию).
* **Навигация:** Небольшие улучшения в логике удаления сообщений и возврата в главное меню в некоторых модулях (`traffic`, `vless`).

#### 🔧 Исправлено:
* **Watchdog:** Исправлена потенциальная ошибка декодирования JSON при получении ответа от Telegram API. Уточнены типы исключений при сетевых ошибках. Исправлена логика определения состояния `inactive`/`failed`.
* **i18n:** Исправлена обработка нечисловых `user_id` и ошибок форматирования строк. Добавлена проверка на существование ключа перевода.

---

<p align="center">
  <i>Версия 1.10.12 (Build 38) — Добавлена полная поддержка русского и английского языков (i18n).</i>
</p>
