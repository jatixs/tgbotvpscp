...
### 🚀 Новые функции:
* **Gateway Bot (Шлюз Поддержки):** Внедрён новый модуль для общения с пользователями и клиентами. Теперь вы можете запустить второго, независимого Telegram-бота (указав `ALERT_BOT_TOKEN` в `.env`), который будет служить шлюзом для ваших клиентов.
  ...
  - **Массовые рассылки (Broadcast):** Добавлена возможность отправлять информационные сообщения, фото и видео всем подписчикам Gateway-бота через удобную inline-панель управления.
