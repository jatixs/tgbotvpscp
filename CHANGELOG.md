...
### 🔒 Безопасность и API:
* **Тотальное шифрование:** Все `POST`-запросы от клиента к серверу (включая аутентификацию) теперь шифруются на лету при помощи кастомного XOR-шифрования и Base64, предотвращая перехват чувствительных данных.
* **Server-Sent Events (SSE):** Все `GET`-эндпоинты API переведены на использование потоковой передачи данных (SSE) с шифрованием ответов.
* **Глобальный перехватчик:** Добавлен нативный перехват `fetch` и `Response.json()` в `common.js`, который автоматически расшифровывает входящие пакеты и зашифровывает исходящие, работая абсолютно прозрачно для остального фронтенда.
