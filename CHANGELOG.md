...
<p align="center">
    ...
    <img src="https://img.shields.io/badge/date-Июль%2004%202026-green?style=flat-square" alt="Date Jul 04 2026"/>
...
### � Добавлено / Улучшено:
* **Система Биллинга (Billing):** Внедрена комплексная система отслеживания платежей. Для каждой ноды и мастер-сервера теперь можно задать стоимость аренды, выбрать валюту (€ / $ / ₽) и установить дату следующего платежа.
* **Автоматические Уведомления (Billing):** Фоновая задача `billing_reminders_task` автоматически определяет хостинг-провайдера ноды через `ip-api.com` и отправляет администратору предупреждение за 3 дня до окончания оплаченного периода.
* **Управление Биллингом (Бот):** В меню управления нодами и мастер-сервером добавлена кнопка «Биллинг» (только для Администратора). Через неё можно задать сумму, выбрать валюту и сдвинуть дату платежа прямо из Telegram. Переключатель напоминаний доступен как для каждой ноды отдельно, так и глобально в настройках уведомлений.
* **WebUI (Бейджи Аренды):** В шапке веб-панели и карточках нод появились информативные локализованные бейджи с количеством оставшихся дней до оплаты или статусом «Просрочен!».
* **WebUI (Модальное Окно Аренды):** Добавлено модальное окно с подробностями аренды: имя провайдера, стоимость с валютой и дата ближайшего платежа.
* **БД (Billing — Автоматические Миграции):** В модель `Node` добавлены поля `is_cloud`, `provider_name`, `next_payment_date`, `billing_amount`, `currency`, `reminder_enabled`. Реализован механизм автоматических миграций через `ALTER TABLE` при каждом старте, исключающий потерю данных при обновлении.
* **Деплой (Billing):** Скрипты `deploy.sh` и `deploy_en.sh` обновлены: при обновлении автоматически запускаются `aerich migrate` + `aerich upgrade`, гарантируя корректность схемы БД без ручного вмешательства.
* **Выбор Метода Пинга:** В настройках WebUI добавлен переключатель метода измерения задержки нод: `ICMP` (традиционный) или `HTTP` (`HEAD`-запрос). Метод `HTTP` решает проблему блокировки ICMP-пакетов строгими файрволами.
* **Агент Ноды (Метод Пинга):** Агент `node.py` обновлён с поддержкой обоих методов. Настройка синхронизируется с сервером; агент адаптирует алгоритм замера и формат вывода результата под выбранный метод.
* **Selftest (Пинг):** Вывод значения пинга в диагностике (`selftest`) теперь корректно отражает активный метод замера.
* **WebUI (Монитор Нод — Данные):** В API-ответы и шаблоны монитора нод добавлены поля биллинга (`billing_amount`, `currency`, `next_payment_date`, `reminder_enabled`), обеспечивая полное отображение статуса аренды на странице мониторинга.
* **WebUI (Монитор Нод — SSE):** Устранена проблема с SSE-каналом, обеспечивающим предварительную отрисовку (pre-render) вкладки сервисов на странице мониторинга нод. Данные теперь отображаются сразу при открытии страницы.
* **Локализация и Форматирование:** Исправлены и дополнены ключи i18n для нод, selftest, speedtest и fail2ban. Единицы измерения в выводе результатов переведены на абсолютные значения (MiB/GiB/MHz) для корректного отображения реальных показателей.

### 🔧 Исправлено:
* **WebUI (Биллинг — DOMPurify):** Обработчики кликов на кнопках биллинга переработаны с инлайн-атрибутов `onclick` на динамическую привязку через JS, что предотвращает их удаление защитой `DOMPurify`.
* **WebUI (Шаблоны — master_billing_json):** Добавлена инъекция переменной `master_billing_json` в шаблоны `dashboard.html`, `nodes_monitor.html` и `settings.html`. Её отсутствие приводило к тому, что бейдж биллинга в шапке не отображался.
* **Core / Middlewares:** Исправлена ошибка `AttributeError: 'int' object has no attribute 'tzinfo'` в `CallbackTTLMiddleware` при обработке устаревших callback-запросов с целочисленными временными метками вместо `datetime`.
* **WebUI (Настройки — 500):** Устранена ошибка 500 на эндпоинте `/api/settings/save`. Добавлены недостающие ключи i18n.
* **WebUI (Настройки — Адаптивность):** Раскладка страницы настроек переработана и теперь корректно масштабируется на узких мобильных экранах.
