...
### ✨ Улучшено:
* **WebUI / Интерфейс:** Проведен глобальный рефакторинг UI: все эмодзи заменены на современные SVG-аналоги и CSS-индикаторы для единообразного и стабильного отображения на всех устройствах.
* **WebUI / Логи:** Реализована автоматическая серверная фильтрация логов перед отправкой в веб-интерфейс (вырезаются эмодзи, генерируемые ботом), сохраняя при этом исходные символы в файлах для истории.
* **WebUI / Безопасность:** Добавлена встроенная фильтрация `filter_zalgo` на уровне ядра для предотвращения инъекций нестандартных unicode-символов и искажения текстов.
...
### ✨ Улучшено:
* **WebUI:**
  * **Пасхалка:** 7 кликов по названию сайта в шапке панели.
  * **Индикатор оплаты:** Иконка биллинга в шапке теперь дополнительно подсвечивается цветом (зелёный/жёлтый/красный/серый) в зависимости от статуса ближайшего платежа.
  * **Подвал дашборда:** Плашка «Работает на Python» в карточке аптайма получила иконку языка и аккуратное выравнивание.
  * **AMOLED-тема:** Всплывающие уведомления (toast) и оверлеи загрузки (модальное окно ноды, панель системных логов) теперь используют чистый чёрный фон вместо серо-синего оттенка, соответствуя остальному дизайну AMOLED-режима.
  * **Сетевая активность:** Метки легенды графика трафика переименованы с RX/TX на ВХ/ИСХ (входящий/исходящий) для русскоязычного интерфейса.

### 🐛 Исправлено:
* **Графики трафика:** Исправлено некорректное отображение графика трафика при изменении размера блока с виджетом.
* **Модальные окна:** Устранён баг, из-за которого после закрытия модального окна ноды (или любого другого модального окна) страница могла резко прокручиваться в самое начало вместо того, чтобы оставаться в месте, откуда оно было открыто.
* **Безопасность (WebUI):** В панели «Скрытые блоки» название виджета теперь выводится через безопасный `textContent` вместо `innerHTML`, что исключает потенциальный XSS через названия блоков.
* **Graceful Shutdown:** Исправлена некорректная обработка завершения работы (graceful shutdown) для основного бота и алерт-бота.

### 🔧 Рефакторинг:
* **Логирование:** Все сообщения логирования переведены на английский язык для единообразия и удобства анализа.
* **Локализация логов:** Реализованы локализованные сообщения логирования для улучшения ясности и консистентности.
...
* **WebUI:**
  ...
  * **Мини-графики:** В блоке быстрой статистики добавлены мини-графики нагрузки CPU/RAM/Диска (по аналогии с графиком трафика), а также наглядная визуализация Uptime/Downtime в виде разделенной шкалы «онлайн/простой».
...
### ✨ Улучшено:
* **WebUI:**
  * **Анимации:** Добавлена плавная анимация "дешифровки" данных (IP-адрес, Ping) при загрузке дашборда до получения SSE-ответов. Это позволило избавиться от визуальных скачков и "вспышек" текста при обновлении интерфейса.
  * **Навигация:** Оптимизировано расположение кнопок в навигационной панели по логическим группам (настройки, администрирование, персонализация, профиль). Кнопка "Редактировать интерфейс" скрывает текст на мобильных устройствах для экономии места.
  * **Загрузка:** Лимит на загрузку файлов снижен с 50 МБ до 5 МБ для защиты от исчерпания памяти (DoS-вектор).
* **Архитектура:** Устранена потенциальная утечка памяти (Memory Leak) при управлении фоновыми задачами в оркестраторе модулей.
* **Сетевая логика:** Полностью переработана механика замера внутреннего пинга. Теперь бот не инициирует самостоятельные запросы к нодам (что могло блокироваться файрволами), а берет точные данные о задержке из контрольных пакетов (heartbeats) самой быстрой доступной ноды.

### 🐛 Исправлено:
* **Система (Systemd):** Исправлено аварийное завершение процесса бота по таймауту (`stop-sigterm timed out. Killing.`) при перезагрузке через `systemctl`. Проблема была вызвана race condition в фоновом мониторинге логов (`tail`, `journalctl`), которые мгновенно пересоздавались при выключении.
* **WebUI:**
  * **Вёрстка:** Блок системных логов больше не схлопывается и не меняет свой размер во время первоначальной загрузки страницы.
  * **Адаптивная сетка дашборда:** Блоки в одной строке теперь всегда выравниваются по высоте, а их внутреннее содержимое (список служб, карточки нод, окно системных логов) корректно адаптируется под выделенное пространство и под ручное изменение размера блока, вместо образования пустых «растянутых» областей.
  * **Карточки нод:** Переработано отображение статистики (ЦП/ОЗУ/Диск/трафик) в карточках нод — компактная сетка вместо инлайн-стилей, корректно выглядит при любом размере блока «Управление Нодами».
  * **Добавление ноды:** Устранён конфликт двух реализаций шифрования запросов (`encryptData`) в `common.js` и `dashboard.js` — старая XOR-функция в `dashboard.js` перекрывала корректную AES-функцию, из-за чего сервер не мог расшифровать запрос и возвращал ошибку «Name required» при попытке добавить ноду.
...
* **Безопасность веб-интерфейса:** Внедрены современные стандарты защиты (HTTP Security Headers). Это защищает панель управления от межсайтового скриптинга (XSS), кликджекинга и принудительно включает безопасное соединение (HSTS).
* **Защита от выполнения команд:** Устранены потенциальные уязвимости (Shell Injection) в модулях просмотра логов SSH и системного мониторинга (Top). Теперь любые пользовательские запросы строго изолируются перед выполнением на сервере.
* **Обновление зависимостей:** Актуализированы системные библиотеки для устранения ранее известных уязвимостей, повышения стабильности и скорости работы бота.
* **Тотальное шифрование:** Все отправляемые данные от клиента к серверу (включая авторизацию) теперь шифруются на лету при помощи кастомного шифрования, предотвращая перехват чувствительной информации.
* **Плавность интерфейса (SSE):** Загрузка данных в веб-панели (мониторинг, графики) переведена на непрерывную потоковую передачу данных. Информация обновляется мгновенно, без задержек и лишних запросов к серверу, а все потоки надежно защищены.
* **Невидимая защита:** На стороне браузера внедрен «умный» перехватчик запросов. Он автоматически зашифровывает все исходящие пакеты и расшифровывает ответы сервера, работая абсолютно незаметно для пользователя.
...
* **Alert-bot:** Внедрён новый модуль для общения с пользователями и клиентами. Теперь вы можете запустить второго, независимого Telegram-бота (указав `ALERT_BOT_TOKEN` в `.env`), который будет служить шлюзом для ваших клиентов.
  - **Тикет-система:** Любые сообщения от клиентов пересылаются администратору в главный бот в виде удобных тикетов. Администратор может отвечать на них напрямую в один клик.
  - **Системные уведомления:** Gateway Bot автоматически мониторит статусы нод и рассылает клиентам уведомления со списком серверов при любых неполадках (падение ноды) или их устранении.
  - **Массовые рассылки (Broadcast):** Добавлена возможность отправлять информационные сообщения, фото и видео всем подписчикам Alert-бота через удобную inline-панель управления.
  - **Anti-Flood и Защита:** Встроена строгая защита от спама — бот принимает сообщения не чаще 1 раза в 3 секунды и принудительно требует использовать функцию «Ответить» (Reply) для ведения диалога, защищая админа от флуда.
  - **Управление уведомлениями с Alert-бота:** Удобная кнопка управления уведомлениями как глобально с главного бота для всех клиентов (по нодам), так и на стороне клиентов.
### ✨ Улучшено:
* **Ротация кнопок:** По причине появления дополнительных возможностей у нод, изменен порядок кнопок и возможность перелистывания кнопок (6 кнопок на 1 страницу).
...
* **Защита от XSS (DOMPurify):** Внедрена библиотека `DOMPurify` для строгой фильтрации HTML-контента на клиенте перед отрисовкой динамических элементов (модальные окна, кнопки с индикаторами загрузки).
* **Защита от Open Redirect:** В клиентской SPA-маршрутизации (перехват кликов по ссылкам) добавлена строгая проверка `window.location.origin`, блокирующая скрытые перенаправления на сторонние домены.
* **Безопасность цепочки поставок:** Версии всех GitHub Actions (`checkout`, `setup-python`, `shellcheck`, `gitleaks`, `trivy`) теперь жёстко зафиксированы по полным SHA-хешам коммитов вместо тегов (`@v4`, `@master`).
* **Статический анализ (SAST):** Устранены ложные срабатывания анализатора Semgrep в безопасных вызовах `subprocess_exec` и логгерах.

### ✨ Улучшено (Пользовательский интерфейс):
* **AMOLED Тема:** Добавлена новая глубокая тёмная тема специально для OLED-экранов, экономящая заряд батареи и обеспечивающая идеальный чёрный фон (`#000000`).
* **Адаптивность:** Переработана логика отображения шапки и мобильного меню (кнопка с волшебной палочкой) для разделов «Настройки» и «Мониторинг нод». Элементы корректно скрываются на компьютерах и отображаются на мобильных устройствах с помощью чистого CSS.
...
### 🔧 Исправлено:
* **Загрузка Favicon:** Устранён баг "Request too large" при загрузке пользовательских иконок. Жёсткий лимит WAF на размер тела запроса увеличен с 10KB до 5MB, а также скорректирован лимит Nginx (`client_max_body_size`), что теперь позволяет без проблем передавать изображения в формате Base64.
---
## [1.23.1] - 2026-06-15

### 🐛 Исправления:
* **Logs (Последние события):** Исправлена ошибка, из-за которой кнопка «Последние события» приводила к срабатыванию пасхалки с "неизвестной командой". Проблема заключалась в несовместимости `aiogram.Router` в `logs.py` с новым Memory Orchestrator. Обработчик переведен на стандартную регистрацию в обход роутера, что вернуло корректное проксирование и lazy-loading.
...
### 🔧 Исправлено:
* **Дашборд:** Исправлено отображение зашифрованного значения вместо имени ноды в заголовке модального окна (`dashboard.js` — добавлен вызов `decryptData` для `data.name`).
* **Бот:** Исправлена ошибка линтера F823 в `bot.py` — переменная `_` в цикле `for _, cfg in configurable` конфликтовала с импортированной функцией перевода `_`. Переименовано в `_btn`.
...
* **Авторизация:** Исправлено небезопасное поведение по умолчанию в `core/auth.py` — `is_allowed()` теперь возвращает `False`; добавлены `btn_*`-псевдонимы всех команд в соответствующие списки ролей; исправлена опечатка `"fall2ban"` → `"fail2ban"` в списке root-команд.
...
* **Docker:** Порт веб-сервера закреплён на `127.0.0.1:8080:8080`, что исключает обход WAF и ограничений частоты запросов через прямой внешний доступ. (спасибо [@artemkaa_2001](https://github.com/artemkaa2001)).
...
* **Backups:** Полноценный менеджер бэкапов в боте: категории `Traffic / Config / Logs / Nodes`, ручное создание с отправкой файла в чат, восстановление и удаление.
* **Backups:** Массовое удаление всех бэкапов с подтверждением.
* **Backups:** Экран настройки таймера автобэкапов: шаг 30 сек до 10 минут, далее нелинейное изменение (`x2` / `/2`), кнопка сброса и переключатель `ВКЛ/ОТКЛ`.
* **Xray:** Поддержка обновления ядра Xray для панели **3X-UI** (нативная установка и Docker-контейнер) через команду `x-ui update`. Автодетекция: сначала проверяется нативная установка, затем Docker.
...
* **Backups:** Отображение интервала переведено в человекочитаемый формат (`дни / часы / минуты / секунды`).
...
* **Ноды:** Устранено ложное срабатывание алерта «Нода недоступна» во время запуска Speedtest. Команда Speedtest теперь выполняется в фоновом потоке, не блокируя heartbeat-цикл. То же применимо к команде `update`.
* **Ноды:** Устранено дублирование сообщения с результатом трафика после остановки мониторинга.
...
* **Hotkeys:** Добавлена поддержка горячих клавиш для удобного редактирования текстовых полей и работы с окнами (спасибо [@artemkaa_2001](https://github.com/artemkaa2001)).
...
* **Сквозное шифрование:** Данные сервисов шифруются на бэкенде (AES-256-CBC + Base64) и расшифровываются на фронтенде.
