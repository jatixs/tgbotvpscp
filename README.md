...
<p align="center">
  <b >v1.10.13</b> — надёжный Telegram-бот для мониторинга и управления вашим VPS или выделенным сервером, теперь с <b>модульной архитектурой</b>, поддержкой <b>нескольких языков</b> и улучшенным процессом развертывания.
</p>
...
* 📡 **Сетевая статистика:** Общий трафик и скорость соединения в реальном времени (с защитой от спама).
* 🔔 **Гибкие уведомления:** Настройка получения алертов о превышении порогов ресурсов, SSH-входах и банах Fail2Ban.
* 🧭 **Администрирование:** Обновление VPS (`apt upgrade`), оптимизация системы, перезагрузка сервера, перезапуск сервиса бота.
* ✨ **Умный установщик/обновщик (`deploy.sh`):**
    ...
    * **Установка зависимостей:** Автоматически устанавливает `iperf3`, `python3-yaml` и другие необходимые пакеты.
* 🚀 **Диагностика:** Проверка пинга, запуск теста скорости (**iperf3** с поддержкой российских серверов и улучшенной локализацией), просмотр топ-процессов по CPU.
...
3.  Убедитесь, что на вашем VPS установлены `curl`, `git` и `python3-venv`.
...
Скрипт автоматически установит системные зависимости (`iperf3`, `python3-yaml`), склонирует репозиторий (`git clone`), настроит `venv`, Python-зависимости, `.env`, `.gitignore` и `systemd`-сервисы.
...
│   ├── iperf_servers_cache.json # Кеш серверов iperf3
│   ├── iperf_servers_ru_cache.yml # Кеш российских серверов iperf3
│   └── ..._flag.txt
│
