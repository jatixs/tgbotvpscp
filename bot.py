    ...
    try:
        ...
        if hasattr(module, 'register_handlers'):
            module.register_handlers(dp)
             ...
             logging.warning(f"Модуль '{module.__name__}' не имеет функции register_handlers().")
...
async def show_main_menu(user_id: int, chat_id: int, state: FSMContext, message_id_to_delete: int = None):
    ...
    # get_main_reply_keyboard теперь тоже должна поддерживать i18n (обновим ее позже)
