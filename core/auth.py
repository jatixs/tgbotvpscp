...
def load_users():
    ...
    # ИСПРАВЛЕНИЕ: Мы больше не используем 'global', так как импортировали
    # словари напрямую
    try:
...
def save_users():
    ...
    try:
        ...
        # Убеждаемся, что ID разрешенных пользователей - целые числа (хотя они
        # и так должны быть)
...
def is_allowed(user_id, command=None):
    ...
    admin_only_commands = [
        ...
        # Включаем колбэки, связанные с управлением пользователями
    ...
    # Для безопасности предположим, что неперечисленные команды требуют прав
    # админа
    if is_admin_group:
        logging.warning(
...
async def refresh_user_names(bot: Bot):
    ...
    needs_save = False
    # Итерируем по копии ключей на случай, если словарь изменится во время
    # итерации (здесь маловероятно, но хорошая практика)
    ...
    for uid in user_ids_to_check:
        ...
        # 4. Имя - "Главный Админ" (на случай, если админ сменил имя/юзернейм в
        # TG).
        should_refresh = (
            not current_name
        ...
        if should_refresh:
            ...
            try:
                ...
                if "chat not found" in str(e).lower(
                    ...
                    # Убеждаемся, что запасное имя установлено, если текущее
                    # невалидно/отсутствует
...
async def get_user_name(bot: Bot, user_id: int) -> str:
    ...
    if cached_name and not cached_name.startswith(
            "Новый_") and not cached_name.startswith("ID: "):
        return cached_name
...
async def send_access_denied_message(
    ...
    if ADMIN_USERNAME:
        ...
        # Текст не передается в этой ссылке
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(
        text="📤 Отправить свой ID администратору", url=admin_link)]])
    try:
