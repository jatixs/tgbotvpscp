...
PING_METHOD = DEFAULT_CONFIG["PING_METHOD"]
...
def _apply_system_config(data):
    global TRAFFIC_INTERVAL, BACKUP_INTERVAL, BACKUP_LAST_INTERVAL, SERVICES_INTERVAL, PING_INTERVAL, PING_METHOD, RESOURCE_CHECK_INTERVAL, CPU_THRESHOLD, RAM_THRESHOLD, DISK_THRESHOLD, RESOURCE_ALERT_COOLDOWN, NODE_OFFLINE_TIMEOUT, WEB_METADATA
    if not data:
        return
    ...
    PING_METHOD = data.get("PING_METHOD", DEFAULT_CONFIG["PING_METHOD"])
...
async def save_system_config_async(new_config: dict):
    ...
    try:
        ...
        if "PING_METHOD" in new_config:
            PING_METHOD = str(new_config["PING_METHOD"])
