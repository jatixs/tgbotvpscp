...
async def send_alert(bot: Bot, message: str, alert_type: str):
    ...
    sent_count = 0
    users_to_alert = []
    for user_id, config_data in ALERTS_CONFIG.items(): # Переименовано во избежание конфликта
        if config_data.get(alert_type, False):
           users_to_alert.append(user_id)

    if not users_to_alert:
        # --- ИЗМЕНЕНО: Используем i18n (язык по умолчанию для логов) ---
        logging.info(_("alert_no_users_for_type", config.DEFAULT_LANGUAGE, alert_type=alert_type))
        # -------------------------------------------------------------
        return

    # --- ИЗМЕНЕНО: Используем i18n (язык по умолчанию для логов) ---
    logging.info(_("alert_sending_to_users", config.DEFAULT_LANGUAGE, alert_type=alert_type, count=len(users_to_alert)))
    # -------------------------------------------------------------
    ...
    for user_id in users_to_alert:
        try:
            ...
            # await bot.send_message(user_id, translated_message, parse_mode="HTML")
            
            # ВРЕМЕННО: Отправляем оригинальное сообщение (которое на языке по умолчанию)
    ...
    # --- ИЗМЕНЕНО: Используем i18n (язык по умолчанию для логов) ---
    logging.info(_("alert_sent_to_users", config.DEFAULT_LANGUAGE, alert_type=alert_type, count=sent_count))
    # -------------------------------------------------------------