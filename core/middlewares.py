...
class SpamThrottleMiddleware(BaseMiddleware):
    async def __call__(
        ...
        is_throttled = (
            ...
            current_action_key is not None and # Не троттлим, если нет ключа (напр. стикер)
            current_action_key == last_action_key # Троттлим только ПОВТОРНЫЕ одинаковые действия
        )
        # ---------------------------

        if is_throttled:
            # Троттлинг активен для ПОВТОРНОГО действия
            logging.info(f"Троттлинг для user_id: {user_id} (повтор '{current_action_key}'). Разница: {current_time - last_timestamp:.2f}с")
            lang = get_user_lang(user_id)

            # Отправляем сообщение о таймауте, если оно еще не было отправлено *для этого периода*
            ...
            if isinstance(event, Message):
                try:
                    ...
                    pass
            # Отвечаем на CallbackQuery без текста (если предупреждение уже было)
        ...
        # Сбрасывать user_throttle_warning_time не нужно, проверка `last_warning_time <= last_timestamp` корректна
