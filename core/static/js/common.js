...
function getHistoryPeriodSeconds(period) {
    const map = {
        '15m': 15 * 60,
        '1h': 60 * 60,
        '6h': 6 * 60 * 60,
    ...
    return Object.prototype.hasOwnProperty.call(map, period) ? map[period] : 24 * 60 * 60;
}
window.getHistoryPeriodSeconds = getHistoryPeriodSeconds;

function prepareHistoryForChart(history, period = '24h') {
    const normalized = Array.isArray(history)
        ? history.filter(point => point && typeof point.t !== 'undefined').sort((a, b) => Number(a.t || 0) - Number(b.t || 0))
        : [];
    ...
    const seconds = getHistoryPeriodSeconds(period);
    let filtered = normalized;

    if (seconds) {
        const latestTs = Number(normalized[normalized.length - 1].t || 0);
        const cutoff = latestTs - seconds;
        filtered = normalized.filter(point => Number(point.t || 0) >= cutoff);
        if (filtered.length === 0) {
            filtered = normalized.slice(-Math.min(60, normalized.length));
        }
    }

    if (filtered.length === 1) {
        const point = filtered[0];
        const fallbackTs = Math.max(0, Number(point.t || 0) - 5);
        return [
            { ...point, t: fallbackTs },
            point,
        ];
...
window.prepareHistoryForChart = prepareHistoryForChart;

function setActiveChartPeriodButtons(containerId, activePeriod) {
    ...
    container.querySelectorAll('.chart-period-btn').forEach(btn => {
        ...
        btn.classList.toggle('dark:bg-black/20', !isActive);
        btn.classList.toggle('text-gray-600', !isActive);
        btn.classList.toggle('dark:text-gray-300', !isActive);
    });
}
window.setActiveChartPeriodButtons = setActiveChartPeriodButtons;
