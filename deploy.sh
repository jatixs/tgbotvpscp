...
update_bot() {
    ...
    msg_warning "ВНИМАНИЕ: Обновление принудительно перезапишет локальные изменения."; msg_warning "Файлы .env, config/, logs/ будут сохранены."; msg_info "1. Получение обновлений из Git (ветка ${branch_to_use})..."
    pushd "${BOT_INSTALL_PATH}" > /dev/null; run_with_spinner "Получение (fetch) изменений" $exec_user_cmd git fetch origin; run_with_spinner "Принудительный сброс (reset) до origin/${branch_to_use}" $exec_user_cmd git reset --hard "origin/${branch_to_use}"; local update_status=$?; popd > /dev/null
...
main_menu() {
    while true; do
        ...
        echo -e "${C_CYAN}  T3)${C_RESET}${C_BOLD}Обновить до Тестовой:${C_RESET}        ${C_CYAN}${TEST_BRANCH_NAME}${C_RESET}"
        echo -e "${C_BLUE}────────────────────────────────────────────────────${C_RESET}"
        echo -e "${C_RED}  4)${C_RESET} ${C_BOLD}Удалить бота${C_RESET}"
        echo -e "  5) ${C_BOLD}Выход${C_RESET}"
        echo "--------------------------------------------------------"
        read -p "$(echo -e "${C_BOLD}Введите номер опции: ${C_RESET}")" choice

        case $choice in
            1) rm -f /tmp/${SERVICE_NAME}_install.log; msg_question "Переустановить (secure, ветка ${GIT_BRANCH})? (y/n): " confirm; if [[ "$confirm" =~ ^[Yy]$ ]]; then uninstall_bot; install_secure "${GIT_BRANCH}"; else msg_info "Отменено."; fi ;;
            2) rm -f /tmp/${SERVICE_NAME}_install.log; msg_question "Переустановить (root, ветка ${GIT_BRANCH})? (y/n): " confirm; if [[ "$confirm" =~ ^[Yy]$ ]]; then uninstall_bot; install_root "${GIT_BRANCH}"; else msg_info "Отменено."; fi ;;
            3) rm -f /tmp/${SERVICE_NAME}_install.log; update_bot "${GIT_BRANCH}" ;;
            T1|t1) rm -f /tmp/${SERVICE_NAME}_install.log; msg_question "Переустановить (secure, ТЕСТОВАЯ ветка ${TEST_BRANCH_NAME})? (y/n): " confirm; if [[ "$confirm" =~ ^[Yy]$ ]]; then uninstall_bot; install_secure "${TEST_BRANCH_NAME}"; else msg_info "Отменено."; fi ;;
            T2|t2) rm -f /tmp/${SERVICE_NAME}_install.log; msg_question "Переустановить (root, ТЕСТОВАЯ ветка ${TEST_BRANCH_NAME})? (y/n): " confirm; if [[ "$confirm" =~ ^[Yy]$ ]]; then uninstall_bot; install_root "${TEST_BRANCH_NAME}"; else msg_info "Отменено."; fi ;;
            T3|t3) rm -f /tmp/${SERVICE_NAME}_install.log; update_bot "${TEST_BRANCH_NAME}" ;;
...
main() {
    ...
    if [ "$INSTALL_STATUS" == "NOT_FOUND" ] || [ "$INSTALL_STATUS" == "PARTIAL" ]; then
        ...
        echo "--------------------------------------------------------"
        read -p "$(echo -e "${C_BOLD}Введите номер опции: ${C_RESET}")" install_choice
        
        local install_done=false; local branch_used=""
        rm -f /tmp/${SERVICE_NAME}_install.log
        
        case $install_choice in
            1) branch_used="${GIT_BRANCH}"; install_secure "$branch_used"; install_done=true ;;
            2) branch_used="${GIT_BRANCH}"; install_root "$branch_used"; install_done=true ;;
            T1|t1) branch_used="${TEST_BRANCH_NAME}"; install_secure "$branch_used"; install_done=true ;;
            T2|t2) branch_used="${TEST_BRANCH_NAME}"; install_root "$branch_used"; install_done=true ;;
            *) msg_info "Установка отменена. Выход."; exit 0 ;;
        esac
        
        if [ "$install_done" = true ]; then
            msg_info "Повторная проверка целостности после установки из ветки '$branch_used'..."
            check_integrity
            if [ "$INSTALL_STATUS" == "OK" ]; then
