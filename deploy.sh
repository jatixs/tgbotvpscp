...
check_integrity() { # ... (код функции без изменений) ...
    ...
    if [ ! -d "${BOT_INSTALL_PATH}/.git" ]; then errors+=("- Отсутствует .git"); INSTALL_STATUS="PARTIAL"; fi
    if [ ! -f "${BOT_INSTALL_PATH}/bot.py" ] || [ ! -f "${BOT_INSTALL_PATH}/watchdog.py" ] || [ ! -d "${BOT_INSTALL_PATH}/core" ] || [ ! -d "${BOT_INSTALL_PATH}/modules" ]; then errors+=("- Отсутствуют основные файлы"); INSTALL_STATUS="PARTIAL"; fi
...
# --- Функции установки ---
# (Без изменений)
install_extras() { # ... (код функции без изменений) ...
...
common_install_steps() { # ... (код функции без изменений) ...
...
install_logic() { # ... (код функции без изменений) ...
    ...
    msg_info "6. Настройка .env..."; msg_question "Token: " T; msg_question "Admin ID: " A; msg_question "Admin Username (opt): " U; msg_question "Bot Name (opt): " N
    ...
    local ip=$(curl -s 4.ipinfo.io/ip || echo "YOUR_IP"); echo ""; echo "---"; msg_success "Установка завершена!"; msg_info "IP: ${ip}"; echo "---"
}
install_secure() { echo -e "\n${C_BOLD}=== Secure Install (branch: ${GIT_BRANCH}) ===${C_RESET}"; common_install_steps; install_logic "secure" "${GIT_BRANCH}"; }
install_root() { echo -e "\n${C_BOLD}=== Root Install (branch: ${GIT_BRANCH}) ===${C_RESET}"; common_install_steps; install_logic "root" "${GIT_BRANCH}"; }
create_and_start_service() { # ... (код функции без изменений) ...
...
uninstall_bot() { # ... (код функции без изменений) ...
    echo -e "\n${C_BOLD}=== Uninstall Bot ===${C_RESET}"; msg_info "1. Stopping services..."; if systemctl list-units --full -all | grep -q "${SERVICE_NAME}.service"; then sudo systemctl stop ${SERVICE_NAME} &> /dev/null; sudo systemctl disable ${SERVICE_NAME} &> /dev/null; fi; if systemctl list-units --full -all | grep -q "${WATCHDOG_SERVICE_NAME}.service"; then sudo systemctl stop ${WATCHDOG_SERVICE_NAME} &> /dev/null; sudo systemctl disable ${WATCHDOG_SERVICE_NAME} &> /dev/null; fi
    msg_info "2. Removing system files..."; sudo rm -f "/etc/systemd/system/${SERVICE_NAME}.service"; sudo rm -f "/etc/systemd/system/${WATCHDOG_SERVICE_NAME}.service"; sudo rm -f "/etc/sudoers.d/98-${SERVICE_NAME}-root"; sudo rm -f "/etc/sudoers.d/99-${WATCHDOG_SERVICE_NAME}-restart"; sudo systemctl daemon-reload
    msg_info "3. Removing bot directory..."; sudo rm -rf "${BOT_INSTALL_PATH}"; msg_info "4. Removing user '${SERVICE_USER}'..."; if id "${SERVICE_USER}" &>/dev/null; then sudo userdel -r "${SERVICE_USER}" &> /dev/null || msg_warning "Failed to fully remove user ${SERVICE_USER}."; fi; msg_success "Uninstall complete."
}
update_bot() { # ... (код функции без изменений) ...
    echo -e "\n${C_BOLD}=== Update Bot (branch: ${GIT_BRANCH}) ===${C_RESET}"; if [ ! -d "${BOT_INSTALL_PATH}/.git" ]; then msg_error "Git repo not found. Cannot update."; return 1; fi
    ...
    msg_warning "Update will overwrite local changes."; msg_warning ".env, config/, logs/ are preserved."; msg_info "1. Fetching updates (branch ${GIT_BRANCH})..."
    pushd "${BOT_INSTALL_PATH}" > /dev/null; run_with_spinner "Git fetch" $exec_user git fetch origin; run_with_spinner "Git reset --hard" $exec_user git reset --hard "origin/${GIT_BRANCH}"; local st=$?; popd > /dev/null
    if [ $st -ne 0 ]; then msg_error "Git update failed."; return 1; fi; msg_success "Project files updated."
    msg_info "2. Updating Python deps..."; run_with_spinner "Pip install" $exec_user "${VENV_PATH}/bin/pip" install -r "${BOT_INSTALL_PATH}/requirements.txt" --upgrade; if [ $? -ne 0 ]; then msg_error "Pip install failed."; return 1; fi
    msg_info "3. Restarting services..."; if sudo systemctl restart ${SERVICE_NAME}; then msg_success "${SERVICE_NAME} restarted."; else msg_error "Failed to restart ${SERVICE_NAME}."; return 1; fi; sleep 1
...
# --- Меню управления ---
main_menu() {
    # --- [ИЗМЕНЕНИЕ] Получаем версии перед циклом ---
    ...
    while true; do
        ...
        if [ -z "$orig_arg1" ] && [ "$GIT_BRANCH" == "main" ]; then
             ...
             echo -e "  ${C_YELLOW} sudo bash $0 <имя_ветки>)${C_RESET}" # Улучшенная подсказка
        fi
        ...
        case $choice in
            1) rm -f /tmp/${SERVICE_NAME}_install.log; msg_question "Переустановить (secure, ${GIT_BRANCH})? (y/n): " confirm; if [[ "$confirm" =~ ^[Yy]$ ]]; then uninstall_bot; install_secure; local_version=$(get_local_version "$README_FILE"); else msg_info "Отменено."; fi ;; # Обновляем локальную версию после переустановки
            2) rm -f /tmp/${SERVICE_NAME}_install.log; msg_question "Переустановить (root, ${GIT_BRANCH})? (y/n): " confirm; if [[ "$confirm" =~ ^[Yy]$ ]]; then uninstall_bot; install_root; local_version=$(get_local_version "$README_FILE"); else msg_info "Отменено."; fi ;; # Обновляем локальную версию после переустановки
            3) rm -f /tmp/${SERVICE_NAME}_install.log; update_bot && local_version=$(get_local_version "$README_FILE") ;; # Обновляем локальную версию после обновления
...
# --- Главный "Роутер" ---
# (Без изменений)
main() { # ... (код функции без изменений) ...
    clear
    ...
    if [ "$INSTALL_STATUS" == "NOT_FOUND" ] || [ "$INSTALL_STATUS" == "PARTIAL" ]; then
        ...
        msg_info "Бот не установлен. Мастер установки..."; echo "---"; echo -e "${C_GREEN}1) Secure ${GIT_BRANCH}${C_RESET}"; echo -e "${C_YELLOW}2) Root ${GIT_BRANCH}${C_RESET}"; echo -e "3) Выход"; echo "---"
        read -p "$(echo -e "${C_BOLD}Опция [1-3]: ${C_RESET}")" install_choice
        local install_done=false; rm -f /tmp/${SERVICE_NAME}_install.log
