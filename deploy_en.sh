...
README_FILE="${BOT_INSTALL_PATH}/README.md" # --- ADDED ---

# --- GitHub Repository and Branch ---
GITHUB_REPO="jatixs/tgbotvpscp"
# Use $orig_arg1 if it exists, otherwise 'main'.
...
run_with_spinner() {
    ...
    if [ $exit_code -ne 0 ]; then
        msg_error "Error during '$msg'. Exit code: $exit_code"
        msg_error "See log for details: /tmp/${SERVICE_NAME}_install.log"
    # else
    #    msg_success "'$msg' completed." # Can be uncommented for debugging
    fi
    return $exit_code
}
...
else msg_error "Neither wget nor curl found."; exit 1; fi
...
get_latest_version() {
    local api_url="$1"
    local latest_tag=$($DOWNLOADER_PIPE "$api_url" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' || echo "API Error")
    if [[ "$latest_tag" == *"API rate limit exceeded"* ]]; then
        latest_tag="API Limit"
    elif [[ "$latest_tag" == "API Error" ]] || [ -z "$latest_tag" ]; then
         latest_tag="Unknown"
    fi
    echo "$latest_tag"
}
...
# --- Integrity Check ---
INSTALL_STATUS="NOT_FOUND" # Internal status, do not translate
STATUS_MESSAGE="Check not performed."
check_integrity() {
    if [ ! -d "${BOT_INSTALL_PATH}" ]; then INSTALL_STATUS="NOT_FOUND"; STATUS_MESSAGE="Bot not installed."; return; fi
    INSTALL_STATUS="OK"; local errors=() # Internal status
    if [ ! -d "${BOT_INSTALL_PATH}/.git" ]; then errors+=("- Missing .git"); INSTALL_STATUS="PARTIAL"; fi # Internal status
    if [ ! -f "${BOT_INSTALL_PATH}/bot.py" ] || [ ! -f "${BOT_INSTALL_PATH}/watchdog.py" ] || [ ! -d "${BOT_INSTALL_PATH}/core" ] || [ ! -d "${BOT_INSTALL_PATH}/modules" ]; then errors+=("- Missing core files"); INSTALL_STATUS="PARTIAL"; fi # Internal status
    if [ ! -f "${VENV_PATH}/bin/python" ]; then errors+=("- Missing venv"); INSTALL_STATUS="PARTIAL"; fi # Internal status
    if [ ! -f "${BOT_INSTALL_PATH}/.env" ]; then errors+=("- (Warning) Missing .env"); fi
    if ! systemctl list-unit-files | grep -q "${SERVICE_NAME}.service"; then errors+=("- Missing ${SERVICE_NAME}.service"); INSTALL_STATUS="PARTIAL"; fi # Internal status
    if ! systemctl list-unit-files | grep -q "${WATCHDOG_SERVICE_NAME}.service"; then errors+=("- Missing ${WATCHDOG_SERVICE_NAME}.service"); INSTALL_STATUS="PARTIAL"; fi # Internal status
    if [ "$INSTALL_STATUS" == "OK" ]; then
        ...
        STATUS_MESSAGE="Installation OK (Bot: ${bot_status} | Watchdog: ${watchdog_status})" # CHANGED
        if [[ " ${errors[*]} " =~ " .env" ]]; then STATUS_MESSAGE+=" ${C_YELLOW}(No .env!)${C_RESET}"; fi
    elif [ "$INSTALL_STATUS" == "PARTIAL" ]; then STATUS_MESSAGE="${C_RED}Installation corrupted.${C_RESET}\n  Issue: ${errors[0]}"; fi
}
...
install_logic() {
    ...
    if [ "$mode" == "secure" ]; then
        msg_info "2. Creating user '${SERVICE_USER}'..."; if ! id "${SERVICE_USER}" &>/dev/null; then sudo useradd -r -s /bin/false -d ${BOT_INSTALL_PATH} ${SERVICE_USER} || exit 1; fi
    ...
    msg_info "4. Setting up venv..."; if [ ! -d "${VENV_PATH}" ]; then run_with_spinner "Creating venv" $exec_user_cmd ${PYTHON_BIN} -m venv "${VENV_PATH}" || exit 1; fi
...
EOF
    msg_info "Starting ${svc}..."; sudo systemctl daemon-reload; sudo systemctl enable ${svc}.service &> /dev/null; run_with_spinner "Starting ${svc}" sudo systemctl restart ${svc}; sleep 2
...
uninstall_bot() {
    ...
    msg_info "2. Removing system files..."
    ...
    msg_info "3. Removing bot directory..."
    sudo rm -rf "${BOT_INSTALL_PATH}"
    msg_info "4. Removing user '${SERVICE_USER}'..."
    if id "${SERVICE_USER}" &>/dev/null; then sudo userdel -r "${SERVICE_USER}" &> /dev/null || msg_warning "Could not completely remove user ${SERVICE_USER}."; fi
...
update_bot() {
    ...
    msg_info "2. Updating Python dependencies..."
...
main_menu() {
    ...
    while true; do
        ...
        echo -e "${C_RED}  4)${C_RESET} ${C_BOLD}Uninstall bot${C_RESET}"
