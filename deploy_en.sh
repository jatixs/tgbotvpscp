...
ensure_data_encryption_key() {
    ...
    export DATA_ENCRYPTION_KEY
    msg_warning "DATA_ENCRYPTION_KEY was missing. Generated a new encryption key."
}
...
install_extras() {
    ...
    if [ "$SERVER_COUNTRY" == "RU" ]; then
        ...
        if ! command -v iperf3 &> /dev/null; then
            run_with_spinner "Installing iperf3" sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -q iperf3
        else
    ...
    for var_entry in "${ENV_VARS[@]}"; do
        ...
        if ! grep -q "^${var_name}=" "${ENV_FILE}"; then
            if [ "$var_name" == "DATA_ENCRYPTION_KEY" ]; then
                ...
                ensure_data_encryption_key
                echo -e "${C_YELLOW}  + Added variable ${var_name}=[REDACTED]${C_RESET}"
...
RUN pip install --no-cache-dir --upgrade pip setuptools wheel && pip install --no-cache-dir docker aiohttp==3.10.11 aiosqlite argon2-cffi sentry-sdk tortoise-orm aerich cryptography tomlkit
    ...
    if [[ "$RESTORE_CHOICE" =~ ^[Yy]$ ]] && [ -f "/tmp/tgbot_env.bak" ]; then
        ...
        if [ -n "$need_bot_token" ] || [ -n "$need_chat_ids" ]; then
            ...
            if [[ "$setup_monitoring" =~ ^[Yy]$ ]]; then
                echo ""
                echo -e "${C_CYAN}Agent monitoring setup:${C_RESET}"
                ...
                if [ -n "$need_bot_token" ]; then
                    read -p "Enter BOT_TOKEN: " saved_bot_token
                fi
