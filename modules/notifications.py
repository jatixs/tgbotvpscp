...
async def cq_toggle_alert(callback: types.CallbackQuery):
    ...
    lang = get_user_lang(user_id)
    # ------------------------------
    if not is_allowed(user_id, "toggle_alert_resources"): # Проверка доступа остается общей
    ...
    try:
        ...
        # --- ИЗМЕНЕНО: Используем i18n ---
        if alert_type == "resources": alert_name = _("notifications_alert_name_res", lang)
        elif alert_type == "logins": alert_name = _("notifications_alert_name_logins", lang)
        else: alert_name = _("notifications_alert_name_bans", lang)
        ...
        if "message is not modified" in str(e): 
            # --- ИЗМЕНЕНО: Используем i18n ---
            await callback.answer(_("users_already_here", lang)) # Используем текст из users
            # --------------------------------
        else:
...
async def parse_ssh_log_line(line: str) -> str | None:
    ...
    if match:
        try:
            ...
            return _("alert_ssh_login_detected", lang, user=user, flag=flag, ip=ip, time=now_time, tz=tz_label)
            # ----------------------------------------------------
        except Exception as e:
...
async def parse_f2b_log_line(line: str) -> str | None:
    ...
    if match:
        try:
            ...
            # --- ИЗМЕНЕНО: Используем i18n (язык по умолчанию) ---
            lang = config.DEFAULT_LANGUAGE
            return _("alert_f2b_ban_detected", lang, flag=flag, ip=ip, time=now_time, tz=tz_label)
            # ----------------------------------------------------
        except Exception as e:
        ...
        try:
            ...
            if cpu_usage >= CPU_THRESHOLD:
                if not RESOURCE_ALERT_STATE["cpu"]:
                    msg = _("alert_cpu_high", lang, usage=cpu_usage, threshold=CPU_THRESHOLD)
                    ...
                    msg = _("alert_cpu_high_repeat", lang, usage=cpu_usage, threshold=CPU_THRESHOLD)
                    alerts_to_send.append(msg)
                    logging.info("Сгенерирован повторный алерт CPU.")
                    LAST_RESOURCE_ALERT_TIME["cpu"] = current_time
            elif cpu_usage < CPU_THRESHOLD and RESOURCE_ALERT_STATE["cpu"]:
                 alerts_to_send.append(_("alert_cpu_normal", lang, usage=cpu_usage))
            ...
            if ram_usage >= RAM_THRESHOLD:
                if not RESOURCE_ALERT_STATE["ram"]:
                    msg = _("alert_ram_high", lang, usage=ram_usage, threshold=RAM_THRESHOLD)
                    alerts_to_send.append(msg)
                    ...
                    msg = _("alert_ram_high_repeat", lang, usage=ram_usage, threshold=RAM_THRESHOLD)
                    alerts_to_send.append(msg)
                    logging.info("Сгенерирован повторный алерт RAM.")
                    LAST_RESOURCE_ALERT_TIME["ram"] = current_time
            elif ram_usage < RAM_THRESHOLD and RESOURCE_ALERT_STATE["ram"]:
                 alerts_to_send.append(_("alert_ram_normal", lang, usage=ram_usage))
            ...
            if disk_usage >= DISK_THRESHOLD:
                if not RESOURCE_ALERT_STATE["disk"]:
                    msg = _("alert_disk_high", lang, usage=disk_usage, threshold=DISK_THRESHOLD)
                    alerts_to_send.append(msg)
                    ...
                    msg = _("alert_disk_high_repeat", lang, usage=disk_usage, threshold=DISK_THRESHOLD)
                    alerts_to_send.append(msg)
                    logging.info("Сгенерирован повторный алерт Disk.")
                    LAST_RESOURCE_ALERT_TIME["disk"] = current_time
            elif disk_usage < DISK_THRESHOLD and RESOURCE_ALERT_STATE["disk"]:
                 alerts_to_send.append(_("alert_disk_normal", lang, usage=disk_usage))
            ...
            if alerts_to_send:
                full_alert_message = "\n\n".join(alerts_to_send)
                await send_alert(bot, full_alert_message, "resources") # send_alert сам обработает язык получателей
    ...
    try:
        async for line in process.stdout:
            try:
                ...
                if line_str:
                    message = await parse_function(line_str) # parse_function уже использует i18n
                    if message: await send_alert(process.bot_ref, message, alert_type)
            except Exception as e:
                logging.error(f"Монитор {alert_type} (stdout): Ошибка парсинга строки: {e}")
    except Exception as e:
        if not isinstance(e, asyncio.CancelledError):
             logging.error(f"Монитор {alert_type} (stdout): Ошибка чтения потока: {e}")
    finally:
        ...
        async for line in process.stderr:
            try:
                ...
                if stderr_str:
                 ...
                 logging.error(f"Монитор {alert_type} (stderr): Ошибка парсинга строки: {e}")
    except Exception as e:
        if not isinstance(e, asyncio.CancelledError):
             logging.error(f"Монитор {alert_type} (stderr): Ошибка чтения потока: {e}")
    finally:
    ...
    try:
        while True:
            ...
            try:
                ...
                logging.warning(f"Процесс 'tail' для {alert_type} (PID: {process.pid}) умер с кодом {return_code}. Ожидание ридеров...")
                
                try:
                    await asyncio.wait_for(asyncio.gather(stdout_closed.wait(), stderr_closed.wait()), timeout=2.0)
                    logging.info(f"Монитор {alert_type}: Оба ридера штатно завершились.")
                except asyncio.TimeoutError:
                    logging.warning(f"Монитор {alert_type}: Таймаут ожидания закрытия ридеров. Принудительная отмена.")
                ...
                if stdout_task and not stdout_task.done():
                    stdout_task.cancel()
                    logging.debug(f"Монитор {alert_type}: Отмена задачи stdout...")
                if stderr_task and not stderr_task.done():
                    stderr_task.cancel()
                    logging.debug(f"Монитор {alert_type}: Отмена задачи stderr...")
                
                if stdout_task: await asyncio.gather(stdout_task, return_exceptions=True)
                if stderr_task: await asyncio.gather(stderr_task, return_exceptions=True)
                
                if process and process.returncode is None: 
                    try: 
                        process.terminate()
                        logging.warning(f"Монитор {alert_type}: Принудительное завершение 'tail' (PID: {process.pid}).")
                    except ProcessLookupError: pass
                
                process = None
                logging.info(f"Монитор {alert_type}: Пауза 5 сек перед перезапуском...")
        ...
        if process and process.returncode is None:
            ...
            try:
                ...
                try:
                    ...
                    logging.warning(f"Монитор {alert_type}: 'tail' (PID: {pid}) не завершился за 2 сек. Попытка kill().")
                    try:
                        process.kill()
             ...
             logging.info(f"Монитор {alert_type}: 'tail' (PID: {pid}) уже был завершен (код: {process.returncode}).")
        else:
