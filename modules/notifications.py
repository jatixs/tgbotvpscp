    ...
    try:
        while True:
            ...
            try:
                ...
                # Сохраняем ссылку на бота в объекте процесса для доступа из _read_stdout
                process.bot_ref = bot 
                ...
                # Даем ридерам 2 секунды, чтобы обработать EOF и закрыться штатно
                try:
                ...
                if process and process.returncode is None: # Если процесс еще жив (маловероятно, но возможно)
                    try: 
                        process.terminate()
                ...
                await asyncio.sleep(5) # Пауза перед перезапуском внешнего цикла while True

    except asyncio.CancelledError:
         logging.info(f"Монитор {alert_type} отменен (штатное завершение).")

    finally:
        # Этот 'finally' сработает только при полной остановке бота (CancelledError)
