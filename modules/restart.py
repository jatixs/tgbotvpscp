        ...
        try:
            ...
            # --------------------------------
             ...
             if "message to edit not found" in str(edit_e):
                  logging.warning(f"Не удалось изменить сообщение об ошибке перезапуска (возможно, удалено): {edit_e}")
             else:
                  logging.error(f"Не удалось изменить сообщение об ошибке перезапуска: {edit_e}")
