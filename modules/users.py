...
async def cq_back_to_manage_users(
...
async def process_add_user_id(message: types.Message, state: FSMContext):
    ...
    if user_id in LAST_MESSAGE_IDS and "manage_users" in LAST_MESSAGE_IDS[user_id]:
        original_question_msg_id = LAST_MESSAGE_IDS[user_id].get(
            "manage_users")
...
async def process_add_user_group(
    ...
    try:
        ...
        # Эта функция (из core/auth.py) сама получит имя, обновит USER_NAMES и
        # вызовет save_users()
        new_user_name = await get_user_name(callback.bot, new_user_id)

        logging.info(
            f"Админ {callback.from_user.id} добавил пользователя {new_user_name} ({new_user_id}) в группу '{group}'")
...
async def cq_delete_user_confirm(callback: types.CallbackQuery):
    ...
    try:
        ...
        save_users()
        logging.info(
            f"Админ {admin_id} удалил пользователя {deleted_user_name} ({user_id_to_delete}) из группы '{deleted_group}'")
...
async def cq_confirm_self_delete(callback: types.CallbackQuery):
    ...
    try:
        ...
        save_users()
        logging.info(
            f"Пользователь {deleted_user_name} ({user_id_to_delete}) удалил себя из группы '{deleted_group}'")
...
async def cq_set_group_existing(
        callback: types.CallbackQuery,
        state: FSMContext):
    # Этот хэндлер вызывается только если мы НЕ в состоянии FSM (т.е. не
    ...
    try:
        ...
        user_name = USER_NAMES.get(
            ...
            f"ID: {user_id_to_change}")
        logging.info(
            f"Админ {admin_id} изменил группу для {user_name} ({user_id_to_change}) с '{old_group}' на '{new_group}'")
